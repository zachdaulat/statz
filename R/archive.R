...
z_dsc <- function(
  ...
  df <- dplyr::select(data, .unit = !!unit_var, .response = !!resp_var)
  ...
  if (!rlang::quo_is_null(bucket_var)) {
    ...
    if (is.character(bucket_expr)) {
      ...
      if (!inherits(df$.time, c("POSIXct", "Date"))) {
        rlang::abort(
          "`time` column must be a datetime object when a string is provided for `bucket`."
        )
      }
  ...
  if (any(is.na(df$.response))) {
    rlang::abort(
      "The response column contains NAs. Please impute or remove missing values."
    )
  }

  if (!treated_unit %in% levels(df$.unit)) {
    rlang::abort(sprintf(
      "Treated unit '%s' not found in the unit column.",
      treated_unit
    ))
  }
  ...
  if (nrow(bucket_counts) != expected_rows) {
    rlang::abort(
      "Unbalanced panel: Not all units are present in every time bucket."
    )
  }
  ...
  if (min_n < 5) {
    cli::cli_warn(
      "The smallest bucket has only {min_n} observations. Low sample sizes increase quantile volatility."
    )
  }
  ...
  df <- switch(
    ...
    shift = dplyr::mutate(
      df,
      .response = .response - unit_loc[as.character(.unit)]
    ),
    scale = dplyr::mutate(
      df,
      .response = .response *
        (unit_loc[[treated_unit]] / unit_loc[as.character(.unit)])
    )
  )
...
print.z_dsc <- function(x, ...) {
  ...
  if (x$diagnostics$converged) {
    cli::cli_alert_success(
      "Optimisation converged in {x$diagnostics$iterations} iterations."
    )
  } else {
    cli::cli_alert_danger(
      "Optimisation failed to converge (Max iter: {x$diagnostics$iterations})."
    )
  }
  ...
  if (x$params$adjust_method == "shift") {
    cli::cli_alert_info(
      "Offset Constant (gamma): {.val {sprintf('%.3f', x$gamma)}}"
    )
    cat("\n")
  }
...
summary.z_dsc <- function(object, ...) {
  ...
  if (object$diagnostics$effective_rank < j) {
    cli::cli_alert_warning(
      "Effective rank is strictly less than J. The raw donor pool is perfectly collinear."
    )
  }
  ...
  if (object$diagnostics$kappa_l2 < object$diagnostics$kappa) {
    cli::cli_alert_success(
      "Ridge penalty successfully improved matrix conditioning."
    )
  }
