...
test_that("statz::lm engines correctly solve simple analytic case", {
  ...
  for (eng in engines) {
    fit <- statz::lm(y ~ x, data = df, engine = eng)
...
test_that("statz::lm strictly fails on NA values", {
  df <- data.frame(
    x = c(1, 2, NA, 4),
    y = c(2, 4, 6, 8)
  )
  ...
  expect_error(statz::lm(y ~ x, data = df, engine = "cholesky"))
  expect_error(statz::lm(y ~ x, data = df, engine = "qr"))
})

test_that("statz::lm engines match stats::lm on palmerpenguins", {
  ...
  for (eng in engines) {
    fit_z <- statz::lm(f, data = penguins_clean, engine = eng)
...
test_that("statz::eigen matches base::eigen for symmetric matrices", {
  ...
  rust_eigen <- statz::eigen(sym_mat)
...
test_that("statz::svd matches base::svd", {
